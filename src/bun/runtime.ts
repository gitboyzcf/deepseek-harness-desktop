...
/** 安装包内置运行时目录(打包后位于 Resources/app/runtime) */
export function bundledRuntimeRoot(): string {
	// 打包/开发构建的运行目录都是 <build>/bin, Resources 是其兄弟目录
	return path.resolve("../Resources/app/runtime");
}

/**
 * 开发期便捷路径: 从 cwd 向上找项目的 resources/runtime
 * (electrobun dev 的 cwd 在 build/<env>-<platform>/<name>-<channel>/bin, 向上 4 级是项目根)
 */
export function devRuntimeRoot(): string | null {
	let dir = process.cwd();
	for (let i = 0; i < 6; i++) {
		const candidate = path.join(dir, "resources", "runtime");
		if (fs.existsSync(path.join(candidate, "dsh"))) return candidate;
...
export function liveRuntimeRoot(userData: string): string {
...
export function inspectRuntime(root: string): RuntimeInfo | null {
	...
	if (
		!fs.existsSync(
			path.join(dshDir, "node_modules", "commander", "package.json"),
		)
	)
		return null;
	...
	try {
		version =
			JSON.parse(fs.readFileSync(dshPkgPath, "utf8")).version ?? version;
	} catch {
		/* 忽略, 按未知版本处理 */
	}
...
/** 优先使用 live(已升级过)副本, 否则回退到安装包内置副本; 开发模式再回退到项目 resources/runtime。DSH_RUNTIME 环境变量可强制指定(调试用) */
export function resolveRuntime(userData: string): RuntimeInfo | null {
	const forced = process.env.DSH_RUNTIME;
	if (forced) {
		const forcedRuntime = inspectRuntime(forced);
		if (forcedRuntime) {
			forcedRuntime.source = "bundled";
			return forcedRuntime;
		}
	}
	const live = inspectRuntime(liveRuntimeRoot(userData));
	if (live) {
		live.source = "live";
		return live;
	}
	...
	const dev = devRuntimeRoot();
	if (dev) {
		const devRuntime = inspectRuntime(dev);
		if (devRuntime) {
			devRuntime.source = "bundled";
			return devRuntime;
		}
	}
	return null;
...
export function materializeLiveRuntime(userData: string): RuntimeInfo {
	const live = liveRuntimeRoot(userData);
	...
	try {
		fs.cpSync(path.join(bundled, "dsh"), path.join(staging, "dsh"), {
			recursive: true,
		});
